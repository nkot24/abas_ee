<!DOCTYPE html>
<html lang="{{ $order->lang ?? 'et' }}">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>{{ ($order->lang ?? 'et') === 'en' ? 'Order Confirmation' : (($order->lang ?? 'et') === 'et' ? 'Tellimuse kinnitus' : 'Užsakymo patvirtinimas') }}</title>
</head>
@php
$en = ($order->lang ?? 'et') === 'en';
$et = ($order->lang ?? 'et') === 'et';
...
$delivery  = $isLocker
    ? ($en ? 'Latvijas Pasts locker' : ($et ? 'Latvijas Pasts pakiautomaadi' : 'Latvijas Pasts pakomāts'))
    : ($isPickup
        ? ($en ? 'Pickup on site' : ($et ? 'Kohapeal kättesaamine' : 'Atsiėmimas vietoje'))
        : ($en ? 'Courier' : ($et ? 'Kuller' : 'Kurjeris')));
@endphp
      ...
      <table width="580" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:4px;overflow:hidden;">
        ...
        <tr>
          <td style="padding:36px 40px 8px;">
            <p style="margin:0 0 6px;font-size:13px;color:#888;letter-spacing:1px;text-transform:uppercase;">
              {{ $en ? 'Order' : ($et ? 'Tellimus' : 'Užsakymas') }} #{{ $order->id }}
            </p>
            <h1 style="margin:0;font-size:26px;font-weight:700;color:#1a1a1a;">
              {{ $en ? 'Thank you for your order!' : ($et ? 'Täname teid tellimuse eest!' : 'Ačiū už jūsų užsakymą!') }}
            </h1>
          </td>
        </tr>
        <tr>
          <td style="padding:12px 40px 28px;">
            <p style="margin:0;font-size:15px;color:#555;line-height:1.6;">
              {{ $en
                ...
                : ($et
                    ? "Tere, {$order->first_name} {$order->last_name}! Teie tellimus on tasutud ja valmistatakse saatmiseks. Võtame teiega ühendust tarnedetailide osas."
        ...
        <tr>
          <td style="padding:28px 40px 8px;">
            <p style="margin:0 0 16px;font-size:11px;font-weight:700;color:#888;letter-spacing:1.5px;text-transform:uppercase;">
              {{ $en ? 'Order Summary' : ($et ? 'Tellimuse kokkuvõte' : 'Užsakymo suvestinė') }}
            </p>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:20px 40px 0;">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td style="padding:6px 0;font-size:14px;color:#888;border-top:1px solid #e8e8e8;">{{ $en ? 'Subtotal' : ($et ? 'Vahesumma' : 'Tarpinė suma') }}</td>
                <td align="right" style="padding:6px 0;font-size:14px;color:#888;border-top:1px solid #e8e8e8;">{{ number_format($order->total, 2) }} €</td>
              </tr>
              <tr>
                <td style="padding:6px 0;font-size:14px;color:#888;">{{ $en ? 'Shipping' : ($et ? 'Tarne' : 'Pristatymas') }} ({{ $delivery }})</td>
                <td align="right" style="padding:6px 0;font-size:14px;color:#888;">{{ number_format($order->shipping_cost, 2) }} €</td>
              </tr>
              <tr>
                <td style="padding:12px 0 8px;font-size:16px;font-weight:700;color:#1a1a1a;border-top:2px solid #1a1a1a;">{{ $en ? 'Total' : ($et ? 'Kokku' : 'Viso') }}</td>
            ...
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td width="50%" style="vertical-align:top;padding-right:20px;">
                  <p style="margin:0 0 4px;font-size:11px;font-weight:700;color:#888;letter-spacing:1px;text-transform:uppercase;">
                    {{ $en ? 'Delivery address' : ($et ? 'Tarneaadress' : 'Pristatymo adresas') }}
                  </p>
                  ...
                  <p style="margin:8px 0 0;font-size:13px;color:#555;">
                    📦 <strong>{{ $en ? 'Latvijas Pasts locker' : ($et ? 'Latvijas Pasts pakiautomaadi' : 'Latvijas Pasts pakomāts') }}:</strong><br>{{ $order->parcel_locker_name }}
                  </p>
                  @elseif($isPickup)
                  <p style="margin:8px 0 0;font-size:13px;color:#555;">
                    🏭 <strong>{{ $en ? 'Pickup address' : ($et ? 'Kättesaamise aadress' : 'Atsiėmimo adresas') }}:</strong><br>{{ $order->parcel_locker_name }}
                  </p>
                  @endif
                </td>
                <td width="50%" style="vertical-align:top;">
                  <p style="margin:0 0 4px;font-size:11px;font-weight:700;color:#888;letter-spacing:1px;text-transform:uppercase;">
                    {{ $en ? 'Contact' : ($et ? 'Kontakt' : 'Kontaktai') }}
                  </p>
                  <p style="margin:0;font-size:14px;color:#444;line-height:1.7;">
                    {{ $order->email }}<br>
                    {{ $order->phone }}
        ...
        <tr>
          <td style="padding:32px 40px;border-top:1px solid #e8e8e8;margin-top:28px;">
            <p style="margin:0;font-size:13px;color:#999;text-align:center;line-height:1.6;">
              {{ $en ? 'If you have any questions, please contact us:' : ($et ? 'Kui teil on küsimusi, võtke meiega ühendust:' : 'Jei turite klausimų, susisiekite su mumis:') }}<br>
